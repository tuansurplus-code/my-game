...
const defaults: WheelSettings = {
  ...
  page_background_color: "#ffffff", button_color: "#e31b23", button_text_color: "#ffffff",
  pointer_color: "#e31b23", center_color: "#e31b23", text_color: "#ffffff", logo_url: "",
  font_family: "Arial, sans-serif", animation_duration: 5.2, header_text: "SINGHAGIRI",
  online_text: "SINGHAGIRI ONLINE", segment_colors: {},
};
...
export default function WheelAppearancePage() {
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      pointer_color: settings.pointer_color, center_color: settings.center_color, text_color: settings.text_color,
      logo_url: settings.logo_url?.trim() || null, font_family: settings.font_family, animation_duration: duration,
      header_text: settings.header_text.trim() || "SINGHAGIRI", online_text: settings.online_text.trim() || "SINGHAGIRI ONLINE",
      segment_colors: normalizedColors, updated_at: new Date().toISOString(),
    };
  ...
  return (
    <main className="page">
      <div className="topbar"><div className="topbar-inner">
        ...
        <div className="top-actions"><button type="button" onClick={() => router.push("/admin")}>← Dashboard</button><button type="button" onClick={logout}>Logout</button></div>
      </div></div>

      <div className="content">
        <div className="heading-row"><div><div className="eyebrow">CUSTOMIZATION</div><h1>Wheel Appearance</h1><p>Customize your Spin & Win experience.</p></div><div className="status-pill"><span /> Live configuration</div></div>
          ...
          <div className="appearance-layout">
            <section className="card editor-card">
              <div className="tabs" role="tablist" aria-label="Wheel appearance sections">
                {tabs.map(tab => <button key={tab.id} type="button" className={`tab ${activeTab === tab.id ? "active" : ""}`} onClick={() => { setActiveTab(tab.id); setMessage(""); setError(""); }} role="tab" aria-selected={activeTab === tab.id}>
                  <span className="tab-icon">{tab.icon}</span><span className="tab-copy"><strong>{tab.label}</strong><small>{tab.description}</small></span>
                </button>)}
              </div>
              <div className="editor-body">
                ...
                <div className="form-footer"><button type="button" className="secondary" onClick={resetDefaults} disabled={saving}>Reset to Default</button><button type="submit" className="primary" disabled={saving}>{saving ? "Saving..." : "Save Changes"}<b>→</b></button></div>
              </div>
            </section>

            <aside className="preview-column">
              <div className="preview-section-head"><div><div className="preview-kicker">PREVIEW</div><h2>Customer View</h2><p>Live preview of the Spin & Win page.</p></div><div className="preview-actions"><button type="button" className="preview-open" onClick={() => window.open("/", "_blank", "noopener,noreferrer")}>Preview ↗</button><span className="live-badge"><i /> Live</span></div></div>
              <div className="preview-frame">
                ...
                <div className="preview" style={{ fontFamily: settings.font_family, background: settings.page_background_color }}>
                  ...
                  <div className="mini-wheel-wrap"><div className="mini-pointer" style={{ color: settings.pointer_color }}>▼</div><div className="mini-wheel"><div className="mini-segments" style={{ background: previewGradient }} /><div className="mini-center" style={{ background: settings.center_color }}><strong style={{ color: settings.text_color }}>SPIN</strong><small style={{ color: settings.text_color }}>& WIN</small></div></div></div>
      ...
      <style jsx>{`
        *{box-sizing:border-box}
        .page{min-height:100vh;background:linear-gradient(145deg,#f6f7f9,#eef1f5);color:#17191d}
        .topbar{height:78px;background:#101114;color:#fff;box-shadow:0 2px 14px rgba(0,0,0,.16)}
        .topbar-inner{height:100%;max-width:1240px;margin:auto;padding:0 24px;display:flex;align-items:center;justify-content:space-between}
        .brand-wrap{display:flex;align-items:center;gap:11px}.brand-mark{width:35px;height:35px;border-radius:10px;background:#e31b23;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:950;box-shadow:0 5px 14px rgba(227,27,35,.3)}
        .brand{font-size:16px;font-weight:950;letter-spacing:.12em}.brand-sub{margin-top:3px;font-size:8px;font-weight:800;letter-spacing:.16em;opacity:.55}.top-actions{display:flex;gap:8px}.top-actions button{border:1px solid #34363b;background:#1b1d21;color:#fff;border-radius:9px;padding:9px 13px;font-size:11px;font-weight:800;cursor:pointer}.top-actions button:hover{background:#292b30}
        .content{max-width:1240px;margin:auto;padding:32px 24px 55px}.heading-row{display:flex;align-items:flex-end;justify-content:space-between;gap:20px}.eyebrow{font-size:9px;font-weight:950;letter-spacing:.16em;color:#e31b23;margin-bottom:6px}.heading-row h1{margin:0;font-size:30px;letter-spacing:-.035em}.heading-row p{margin:7px 0 0;color:#777;font-size:13px}.status-pill{display:flex;align-items:center;gap:7px;padding:8px 11px;border:1px solid #dfe3e8;border-radius:999px;background:#fff;font-size:10px;font-weight:800;color:#666}.status-pill span{width:7px;height:7px;border-radius:50%;background:#18a05e;box-shadow:0 0 0 3px rgba(24,160,94,.12)}
        .alert{margin-top:18px;padding:12px 15px;border-radius:11px;display:flex;gap:9px;align-items:center;font-size:12px;font-weight:800}.success{background:#eaf8ef;border:1px solid #c9ecd5;color:#19703a}.error{background:#fff0f0;border:1px solid #f1c8cd;color:#a51d2d}
        .appearance-layout{display:grid;grid-template-columns:minmax(0,780px) minmax(360px,404px);gap:52px;align-items:start;margin-top:52px;justify-content:center}.card{background:rgba(255,255,255,.98);border:1px solid #e1e5e9;border-radius:17px;box-shadow:0 12px 35px rgba(26,34,46,.07)}
        .editor-card{overflow:hidden;min-width:0}.tabs{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:4px;border-bottom:1px solid #e4e7eb;background:#f7f8fa;padding:10px}.tab{position:relative;min-width:0;border:1px solid transparent;background:transparent;text-align:left;display:flex;align-items:center;gap:9px;min-height:66px;padding:10px 12px;border-radius:11px;color:#70757d;cursor:pointer;transition:.18s}.tab:hover{background:#eceff2;color:#282b30}.tab.active{background:#fff;color:#17191d;border-color:#e4e6e9;box-shadow:0 5px 16px rgba(22,28,35,.07)}.tab.active:after{content:"";position:absolute;left:12px;right:12px;bottom:-11px;height:3px;border-radius:3px;background:#e31b23}.tab-icon{width:31px;height:31px;flex:none;border-radius:8px;background:#eef0f3;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:900}.tab.active .tab-icon{background:#fff0f1;color:#e31b23}.tab-copy{display:flex;flex-direction:column;gap:3px;min-width:0;overflow:hidden}.tab-copy strong{font-size:11px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tab-copy small{font-size:8px;line-height:1.25;color:#90959d;white-space:normal;overflow-wrap:anywhere}
        .editor-body{padding:28px 18px 26px;min-height:580px}.section-head{margin-bottom:20px;text-align:left}.section-head .kicker{font-size:8px;font-weight:950;letter-spacing:.15em;color:#e31b23}.section-head h2{margin:5px 0 0;font-size:19px;letter-spacing:-.02em}.section-head p{margin:6px 0 0;font-size:11px;color:#777}.settings-section{margin-bottom:26px}.settings-section h3{margin:0 0 12px;font-size:12px;font-weight:900;color:#17191d}.settings-box{border:1px solid #e4e7eb;border-radius:13px;background:#fbfcfd;padding:16px}.field-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px 16px}.field{display:flex;flex-direction:column;gap:7px}.field.full{grid-column:1/-1}.field label{font-size:9px;font-weight:850;color:#555b63}.field input:not([type="color"]),.field select{width:100%;height:38px;border:1px solid #dfe3e8;border-radius:8px;background:#fff;padding:0 11px;font-size:11px;color:#17191d;outline:none;appearance:auto}.field input:not([type="color"])::placeholder{color:#9da2a8}.field input:focus,.field select:focus{border-color:#e31b23;box-shadow:0 0 0 3px rgba(227,27,35,.08)}.color-field{display:flex;gap:8px}.color-field input[type=color]{width:46px;height:42px;padding:3px;border:1px solid #dfe3e8;border-radius:9px;background:#fff;cursor:pointer}.color-field input[type=text]{flex:1}.helper{margin:5px 0 0;font-size:10px;color:#92969c}
        .segment-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.segment-row{display:flex;align-items:center;gap:12px;border:1px solid #e4e7eb;background:#fff;border-radius:11px;padding:12px}.segment-number{width:28px;height:28px;border-radius:8px;background:#f1f3f5;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#666}.segment-info{flex:1;min-width:0}.segment-info strong{display:block;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.segment-info small{display:block;margin-top:3px;font-size:9px;color:#92969c}.segment-color{display:flex;align-items:center;gap:7px}.segment-color input[type=color]{width:36px;height:36px;padding:2px;border:1px solid #dfe3e8;border-radius:8px;cursor:pointer}.segment-color code{font-size:9px;color:#777}.empty-state{border:1px dashed #d8dce1;border-radius:12px;padding:28px;text-align:center;color:#858a91;font-size:11px}.range-wrap{display:flex;align-items:center;gap:15px}.range-wrap input[type=range]{flex:1;accent-color:#e31b23}.range-value{min-width:64px;text-align:center;border:1px solid #dfe3e8;border-radius:8px;background:#fff;padding:9px;font-size:11px;font-weight:900}
        .form-footer{display:flex;justify-content:space-between;align-items:center;border-top:1px solid #e7e9ec;margin-top:30px;padding-top:22px}.secondary,.primary{border-radius:9px;padding:11px 16px;font-size:11px;font-weight:850;cursor:pointer}.secondary{border:1px solid #d9dde2;background:#fff;color:#555}.secondary:hover{background:#f6f7f8}.primary{border:0;background:#e31b23;color:#fff;display:flex;align-items:center;gap:18px;box-shadow:0 7px 18px rgba(227,27,35,.2)}.primary:hover{background:#c91820}.primary:disabled,.secondary:disabled{opacity:.55;cursor:not-allowed}.primary b{font-size:15px}
        .preview-column{min-width:0;position:sticky;top:20px}.preview-section-head{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;margin:0 0 13px} .preview-actions{display:flex;align-items:center;gap:7px}.preview-open{border:1px solid #dfe3e8;background:#fff;color:#333;border-radius:999px;padding:7px 10px;font-size:10px;font-weight:850;cursor:pointer}.preview-open:hover{background:#f6f7f8}.preview-kicker{font-size:9px;font-weight:950;letter-spacing:.15em;color:#e31b23}.preview-section-head h2{margin:5px 0 0;font-size:20px;letter-spacing:-.02em}.preview-section-head p{margin:5px 0 0;font-size:11px;color:#777}.live-badge{display:flex;align-items:center;gap:7px;padding:7px 10px;border:1px solid #dfe3e8;border-radius:999px;background:#fff;font-size:10px;font-weight:800;color:#666;white-space:nowrap}.live-badge i{width:7px;height:7px;border-radius:50%;background:#18a05e;box-shadow:0 0 0 3px rgba(24,160,94,.12)}
        .preview-frame{border:1px solid #dfe3e8;border-radius:14px;overflow:hidden;background:#fff;box-shadow:0 10px 28px rgba(26,34,46,.06)}.browser-bar{height:32px;display:flex;align-items:center;gap:6px;padding:0 11px;background:#eef0f2;border-bottom:1px solid #dfe3e8}.browser-bar>span{width:7px;height:7px;border-radius:50%;background:#c7cbd0}.browser-bar>div{margin-left:7px;flex:1;height:19px;display:flex;align-items:center;padding:0 9px;border-radius:5px;background:#fff;border:1px solid #dfe3e8;color:#9a9ea4;font-size:8px}.preview{min-height:500px;padding:18px 15px 22px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center}.preview-header{width:100%;display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;font-size:10px;color:#555}.preview-header b{font-size:12px;letter-spacing:.08em}.preview-header span{font-size:8px;font-weight:900;letter-spacing:.14em;color:#888}.preview-logo{max-width:120px;max-height:42px;object-fit:contain;margin:2px 0 7px}.preview-online{font-size:8px;font-weight:900;letter-spacing:.14em;color:#888;margin-bottom:5px}.preview-title{font-size:25px;font-weight:950;letter-spacing:-.04em;line-height:1.05}.preview-subtitle{font-size:10px;color:#777;margin-top:5px;max-width:300px}.mini-wheel-wrap{position:relative;width:190px;height:190px;margin:18px 0 12px;display:flex;align-items:center;justify-content:center}.mini-pointer{position:absolute;z-index:3;top:-7px;font-size:23px;line-height:1}.mini-wheel{position:relative;width:170px;height:170px;border-radius:50%;border:8px solid #fff;box-shadow:0 5px 18px rgba(0,0,0,.18);display:flex;align-items:center;justify-content:center;overflow:hidden}.mini-segments{position:absolute;inset:0;border-radius:50%}.mini-center{position:relative;z-index:2;width:58px;height:58px;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 3px 9px rgba(0,0,0,.2)}.mini-center strong{font-size:11px;line-height:1}.mini-center small{font-size:8px;font-weight:900;margin-top:2px}.preview-button{border:0;border-radius:8px;padding:10px 25px;font-size:10px;font-weight:900;letter-spacing:.04em;box-shadow:0 5px 14px rgba(0,0,0,.12)}.preview-meta{margin:12px 0 0;display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.preview-meta div{background:#fff;border:1px solid #e1e5e9;border-radius:10px;padding:9px 4px;text-align:center}.preview-meta b{display:block;font-size:13px}.preview-meta span{display:block;margin-top:3px;font-size:8px;color:#888}.preview-note{text-align:center;margin:9px 0 0;font-size:9px;color:#8a8f96}
        .loading-page{display:flex;align-items:center;justify-content:center;background:#f5f6f8}.loading-card{background:#fff;border:1px solid #e1e5e9;border-radius:14px;padding:22px 28px;font-size:12px;font-weight:800;box-shadow:0 10px 30px rgba(0,0,0,.06)}
        @media(max-width:1050px){.appearance-layout{grid-template-columns:minmax(0,1fr) 340px;gap:24px}.tab{padding:10px 8px;gap:7px}.tab-icon{width:28px;height:28px;font-size:14px}.tab-copy strong{font-size:10px}.tab-copy small{font-size:7.5px}}
        @media(max-width:800px){.content{padding:24px 16px 40px}.topbar-inner{padding:0 16px}.heading-row{align-items:flex-start;flex-direction:column}.status-pill{display:none}.appearance-layout{display:block}.preview-column{position:static;margin-top:22px}.tabs{grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}.tab{justify-content:flex-start;min-height:62px;padding:10px}.tab.active:after{left:10%;right:10%;bottom:-7px}.tab-copy small{display:block;font-size:8px}.editor-body{padding:24px 18px}.field-grid,.segment-list{grid-template-columns:1fr}.field.full{grid-column:auto}.form-footer{gap:10px}.secondary,.primary{flex:1;justify-content:center}.preview-frame{max-width:100%}}
        @media(max-width:480px){.brand-sub{display:none}.brand{font-size:13px}.top-actions button{padding:8px 9px}.heading-row h1{font-size:25px}.tab-icon{width:30px;height:30px}.tab-copy strong{font-size:10px}.editor-body{padding:20px 14px}.settings-box{padding:13px}.preview{min-height:390px}.mini-wheel-wrap{transform:scale(.88);margin:10px 0 2px}.preview-meta{gap:5px}.preview-meta div{padding:8px 3px}}
      `}</style>
    </main>
  );
}
...
function AnimationTab({ settings, updateSetting }: { settings: WheelSettings; updateSetting: <K extends keyof WheelSettings>(key: K, value: WheelSettings[K]) => void }) {
  return <>
    ...
    <div className="settings-section"><h3>Spin Duration</h3><div className="settings-box"><div className="field">
      <label>Animation Duration</label><div className="range-wrap"><input type="range" min="1" max="10" step="0.1" value={settings.animation_duration} onChange={e => updateSetting("animation_duration", Number(e.target.value))} /><div className="range-value">{Number(settings.animation_duration).toFixed(1)} sec</div></div>
