...
const defaults: WheelSettings = {
  title: "SPIN & WIN", subtitle: "Spin daily and win exciting rewards!", button_text: "SPIN NOW",
  page_background_color: "#ffffff", button_color: "#e31b23", button_text_color: "#ffffff",
  pointer_color: "#e31b23", center_color: "#e31b23", text_color: "#ffffff", logo_url: null,
  font_family: "Arial, sans-serif", animation_duration: 5.2,
};
...
export default function SpinGame({ settings = defaults }: { settings?: WheelSettings }) {
  ...
  function normalizeMobile(value: string) {
    ...
    if (number.startsWith("94")) number = "0" + number.slice(2);
    return number;
  }
  ...
  function createSegment(index: number, total: number, radius: number) {
    const startAngle = (360 / total) * index, endAngle = (360 / total) * (index + 1);
    const start = polarToCartesian(200, 200, radius, endAngle), end = polarToCartesian(200, 200, radius, startAngle);
    const largeArcFlag = endAngle - startAngle > 180 ? 1 : 0;
    return `M 200 200 L ${start.x} ${start.y} A ${radius} ${radius} 0 ${largeArcFlag} 0 ${end.x} ${end.y} Z`;
  }
  function getTextPosition(index: number, total: number) {
    return polarToCartesian(200, 200, 125, (360 / total) * index + 180 / total);
  }
  ...
  async function spin() {
    ...
    if (!prizes.length) { setMessage("No prizes are currently available."); return; }
    const normalized = normalizeMobile(mobile);
    if (!/^07\d{8}$/.test(normalized)) { setMessage("Please enter a valid Sri Lankan mobile number."); return; }
    setBusy(true);
    const { data, error } = await supabase.rpc(process.env.NEXT_PUBLIC_SPIN_RPC || "spin_and_win", { p_mobile: normalized });
    if (error) {
      console.error("Spin error:", error); setBusy(false);
      const errorText = error.message?.toUpperCase() || "";
      if (errorText.includes("ALREADY") || errorText.includes("DUPLICATE") || errorText.includes("UNIQUE")) setMessage("You have already used your spin today.");
      else setMessage("Unable to complete the spin. Please try again.");
      return;
    }
    const result: WinResult = Array.isArray(data) ? data[0] : data;
    if (!result) { setBusy(false); setMessage("No result was returned. Please try again."); return; }
    const total = prizes.length, segmentAngle = 360 / total, winnerIndex = Number(result.segment_index);
    const winnerCenter = winnerIndex * segmentAngle + segmentAngle / 2;
    const extraRotation = 360 * 7, finalRotation = 360 - winnerCenter;
  ...
  async function copyCoupon() {
    ...
    try { await navigator.clipboard.writeText(winner.coupon_code); setCopied(true); window.setTimeout(() => setCopied(false), 2000); }
    catch { setMessage("Unable to copy coupon code."); }
  }

  const s = settings || defaults;
  const safeDuration = Number.isFinite(Number(s.animation_duration)) ? Number(s.animation_duration) : 5.2;
  const pageStyle = { fontFamily: s.font_family, backgroundColor: s.page_background_color };

  return (
    <div className="spin-section" style={pageStyle}>
      {s.logo_url && <img className="game-logo" src={s.logo_url} alt="Singhagiri" />}
      <h1 className="game-title">{s.title}</h1>
      <p className="game-subtitle">{s.subtitle}</p>
      ...
      <div className="wheel-container">
        ...
        <div className="wheel" style={{ transform: `rotate(${rotation}deg)`, transitionDuration: `${safeDuration}s` }}>
          {loadingPrizes ? <div className="wheel-loading">Loading prizes...</div> : !prizes.length ? <div className="wheel-loading">No prizes</div> : (
            <svg viewBox="0 0 400 400" className="wheel-svg">
              {prizes.map((prize, index) => {
                const total = prizes.length, position = getTextPosition(index, total), angle = (360 / total) * index + 180 / total;
                return <g key={prize.id}>
                  <path d={createSegment(index, total, 190)} className={`wheel-segment segment-${index % 6}`} />
              ...
              <circle cx="200" cy="200" r="43" className="wheel-center" />
              <circle cx="200" cy="200" r="34" className="wheel-center-inner" style={{ fill: s.center_color }} />
              <text x="200" y="197" textAnchor="middle" className="spin-center-text" style={{ fill: s.text_color }}>SPIN</text>
              <text x="200" y="215" textAnchor="middle" className="spin-center-sub" style={{ fill: s.text_color }}>&amp; WIN</text>
            </svg>
          )}
        </div>
